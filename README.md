...
MODEL_DIR=artifacts/ensemble uv run uvicorn deep_ecg.serving.api:app --port 8000
```

The Docker image installs ONNX Runtime and FastAPI only, no PyTorch, and copies
`artifacts/` in. Mount a different bundle and point `MODEL_DIR` at it to serve
...
order (I, II, III, aVR, aVL, aVF, V1–V6) at any stated sampling rate. The service
