...
The bundle mirrors a real one (ONNX graph + metadata + lead statistics) but uses
a small randomly initialized model, so the serving tests never depend on a
trained checkpoint or on PTB-XL being present.
"""
...
from deep_ecg.serving.export import export_onnx
...
def tiny_bundle(tmp_path):
    ...
    out = tmp_path / "bundle"
    out.mkdir()
    export_onnx(model, TARGET_LEN, out / "model_0.onnx")
    metadata = BundleMetadata(
        ...
        models=[ModelSpec(onnx="model_0.onnx")],
    )
